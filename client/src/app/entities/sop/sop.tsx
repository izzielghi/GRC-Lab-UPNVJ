...
import { getEntities, reset } from './sop.reducer';
...
export const SOP = () => {
  ...
  const sOPList = useAppSelector(state => state.sOP.entities);
  const loading = useAppSelector(state => state.sOP.loading);
  const links = useAppSelector(state => state.sOP.links);
  ...
  return (
    <div>
      <h2 id="sop-heading" data-cy="SOPHeading">
        SOPS
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
          </Button>
          <Link to="/sop/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={sOPList ? sOPList.length : 0}
          next={handleLoadMore}
          hasMore={paginationState.activePage - 1 < links.next}
          loader={<div className="loader">Loading ...</div>}
        >
          {sOPList && sOPList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('documentPath')}>
                    Document Path <FontAwesomeIcon icon={getSortIconByFieldName('documentPath')} />
                  </th>
              ...
              <tbody>
                {sOPList.map((sOP, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    <td>
                      <Button tag={Link} to={`/sop/${sOP.id}`} color="link" size="sm">
                        {sOP.id}
                      </Button>
                    </td>
                    <td>{sOP.title}</td>
                    ...
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button tag={Link} to={`/sop/${sOP.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                          <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Ubah</span>
                        </Button>
                        <Button
                          onClick={() => (window.location.href = `/sop/${sOP.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && <div className="alert alert-warning">No SOPS found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
