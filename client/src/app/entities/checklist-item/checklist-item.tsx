...
import { getEntities, reset } from './checklist-item.reducer';
...
export const ChecklistItem = () => {
  ...
  const checklistItemList = useAppSelector(state => state.checklistItem.entities);
  const loading = useAppSelector(state => state.checklistItem.loading);
  const links = useAppSelector(state => state.checklistItem.links);
  ...
  return (
    <div>
      <h2 id="checklist-item-heading" data-cy="ChecklistItemHeading">
        Checklist Items
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
          </Button>
          <Link to="/checklist-item/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={checklistItemList ? checklistItemList.length : 0}
          next={handleLoadMore}
        ...
        >
          {checklistItemList && checklistItemList.length > 0 ? (
              ...
              <tbody>
                {checklistItemList.map((checklistItem, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    <td>
                      <Button tag={Link} to={`/checklist-item/${checklistItem.id}`} color="link" size="sm">
                        {checklistItem.id}
                      </Button>
                    </td>
                    ...
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`/checklist-item/${checklistItem.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                          <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">Lihat</span>
                        </Button>
                        <Button
                          tag={Link}
                          to={`/checklist-item/${checklistItem.id}/edit`}
                          color="primary"
                          size="sm"
                        ...
                        <Button
                          onClick={() => (window.location.href = `/checklist-item/${checklistItem.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && <div className="alert alert-warning">No Checklist Items found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
